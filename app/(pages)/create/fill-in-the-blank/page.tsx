...
import { FillInTheBlankWithoutObjectIdType, FillInTheBlankLineType } from '@/lib/types';
...
export default function Page() {
	...
	const [lines, setLines] = useState<FillInTheBlankLineType[]>([
		{
			blankIndices: [],
			text: "",
		}
	...
	const handleUpload = async () => {
		try {
			...
			const res = await axios.put("/api/fill-in-the-blank", data)
			if (res.status != 200)
	...
	return (
		<Card className="p-4 max-w-2xl mx-auto">
			...
			<CardContent>
				{lines.map((line: FillInTheBlankLineType, index) => (
					<Line
						lines={lines}
						setLines={setLines}
